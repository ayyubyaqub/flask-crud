...
def employees_view():
    if request.method == 'GET':
        ...
        employees_data = EmployeeSchema().dump(employees, many=True)
        return make_response(jsonify(employees_data), 200)

    elif request.method == 'POST':
        employee, errors = EmployeeSchema().load(request.json)
        if errors:
            return make_response(jsonify(errors), 400)
        db.session.add(employee)
        db.session.commit()
        return make_response(jsonify(EmployeeSchema().dump(employee)), 201)
...
def employee_view(id):
    ...
    if request.method == 'GET':
        return make_response(jsonify(EmployeeSchema().dump(employee)), 200)

    elif request.method == 'PUT':
        employee, errors = EmployeeSchema().load(request.json, instance=employee, partial=True)
        if errors:
            return make_response(jsonify(errors), 400)
        db.session.commit()
        return make_response(jsonify(EmployeeSchema().dump(employee)), 200)
